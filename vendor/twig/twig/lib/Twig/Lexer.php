...
class Twig_Lexer
{
    ...
    private function lexData()
    {
        ...
        if ($this->position == count($this->positions[0]) - 1) {
            $this->pushToken(/* Twig_Token::TEXT_TYPE */
    ...
    private function lexExpression()
    {
        ...
        if (preg_match($this->regexes['operator'], $this->code, $match, null, $this->cursor)) {
            $this->pushToken(/* Twig_Token::OPERATOR_TYPE */
                8, preg_replace('/\s+/', ' ', $match[0]));
            $this->moveCursor($match[0]);
        } // names
        elseif (preg_match(self::REGEX_NAME, $this->code, $match, null, $this->cursor)) {
            $this->pushToken(/* Twig_Token::NAME_TYPE */
                5, $match[0]);
            $this->moveCursor($match[0]);
        } // numbers
        elseif (preg_match(self::REGEX_NUMBER, $this->code, $match, null, $this->cursor)) {
            ...
            $this->pushToken(/* Twig_Token::NUMBER_TYPE */
                6, $number);
            $this->moveCursor($match[0]);
        } // punctuation
        elseif (false !== strpos(self::PUNCTUATION, $this->code[$this->cursor])) {
            ...
            $this->pushToken(/* Twig_Token::PUNCTUATION_TYPE */
                9, $this->code[$this->cursor]);
            ++$this->cursor;
        } // strings
        elseif (preg_match(self::REGEX_STRING, $this->code, $match, null, $this->cursor)) {
            $this->pushToken(/* Twig_Token::STRING_TYPE */
